<template>
  <div class="list">
    <!-- 面包屑导航区域 -->
    <!-- 一般情况下它用来表示我们当前所处的站点位置，也可以帮助我们能够更快的回到上个层级 -->
    <!-- 设置 separator-class 可使用相应的 iconfont 作为分隔符 -->
      ...
      <el-table :data="userlist" border stripe>
        ...
        <el-table-column align="center" label="类别">
          <template slot-scope="scope">
            ...
            <span v-if="scope.row.num == 3">盒饭</span>
            <span v-if="scope.row.num == 4">其他</span>
            <span v-else>{{ scope.row.key }}</span>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="150px">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              icon="el-icon-delete"
              size="mini"
              @click="removeUserById(scope.row.star_id)"
            ></el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    async removeUserById(star_id) {
      ...
      const { data: res } = await this.$http.get("delUserStar", {
        params: {
          star_id,
        },
      });
    ...
    async getUserSearch() {
      const { data: res } = await this.$http.get("search3", {
        params: {
          // 前面是啥，就按照啥搜索
